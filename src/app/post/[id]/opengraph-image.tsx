...
export default async function Image({ params }: { params: { id: string | number } }) {
  ...
  const { description } = mainNote ?? {}
  const images = mainNote?.images ?? []
  const currentImage = images?.[0]

  const urlImage = currentImage?.attributes

  const arrSemiBold = await promises.readFile(process.cwd() + "/public/fonts/Inter-SemiBold.ttf")
  const arrLight = await promises.readFile(process.cwd() + "/public/fonts/Inter-Light.ttf")
