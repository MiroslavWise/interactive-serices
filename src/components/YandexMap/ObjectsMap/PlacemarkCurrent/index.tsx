...
const PlacemarkCurrentStates: TPlacemarkCurrent = ({ coordinates, id, idUser, dispatch, offer }) => {
    return coordinates.map((item) => <Place key={`${item[0]}-${item[1]}-${id}`} item={item} id={id} idUser={idUser} dispatch={dispatch} offer={offer} />)
}
    ...
    return (
        <Placemark
            ...
            options={{
                ...
                zIndexActive: 50,
                iconColor:
                    offer?.provider === "alert" ? "#eb3f5e" : offer?.provider === "offer" ? "#a26be8" : offer?.provider === "discussion" ? "#ee4e29" : "#000",
            }}
            onClick={(event: any) => {
