...
export const PlacemarkCurrent: TPlacemarkCurrent = ({
  ...
  useEffect(() => {
    if (isActive) {
      const buttonHistory = document.getElementById(`button-on-history-${id}`)
      const buttonMessage = document.getElementById(`button-message-${id}`)
    ...
    <div class="maps-content">
      <div class="maps-service-title-block">
        <div class="maps-photo-block">
          <img src="${image.url ? image.url : "/png/default_avatar.png"}" alt="avatar" width="400" height="400" class="maps-avatar">
  ...
  return (
    <Placemark
      ...
      onClick={(e: any) => {
        setTimeout(() => {
          if (e?.originalEvent?.map?.balloon?._balloon?._state === "OPEN") {
            setIsActive(true)
          } else {
            setIsActive(false)
          }
