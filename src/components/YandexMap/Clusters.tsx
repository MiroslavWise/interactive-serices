...
function Clusters() {
  ...
  React.useEffect(() => {
    if (ymaps3) {
      Promise.all([ymaps3.import("@yandex/ymaps3-reactify"), ymaps3.ready]).then(async ([{ reactify }]) => {
        const r = reactify.bindTo(React, ReactDOM)
        const cluster = r.module(await ymaps3.import("@yandex/ymaps3-clusterer"))
        //@ts-ignore
        const grid = cluster.clusterByGrid({ gridSize: 64 })
        setCluster(cluster)
        setGrid(grid)

        setReactifiedApi(r.module(ymaps3))
      })
    }
  }, [])
  ...
  if (!reactifiedApi || !Cluster) return null

  const { YMapMarker } = reactifiedApi ?? {}
  const { YMapClusterer } = Cluster ?? {}
