...
export const PopupMenu: TPopupMenu = memo(function $PopupMenu({ dataUser, isBarter }) {
    ...
    const geo = useMemo(() => {
        ...
        return dataUser?.addresses?.find((item) => item?.addressType === "main")?.additional
    }, [dataUser?.addresses])

    return (
        <div className={styles.wrapper} data-is-barter={isBarter} data-visible={isVisible}>
            ...
            {isBarter ? (
                <motion.div data-is-open={isVisible} initial={{ borderRadius: 12 }} data-menu>
                    <div data-user>
                        <NextImageMotion src={dataUser?.profile?.image?.attributes?.url!} alt="avatar" width={250} height={250} />
                        <div data-title>
                            <h2 data-title>
                                {dataUser?.profile?.firstName || "--"} {dataUser?.profile?.lastName || "--"}
                            </h2>
                            {geo ? <GeoTagging location={geo} fontSize={12} size={16} /> : null}
                        </div>
                    </div>
                    ...
                    >
                        <Image src="/svg/arrow-left.svg" alt="back" width={20} height={20} unoptimized />
                        <p>Назад</p>
                    </li>
                </motion.div>
            ) : (
