    ...
    >
      <section className="w-full md:max-w-[33.75rem] flex flex-col gap-[1.875rem] items-center rounded-t-3xl md:rounded-[2rem] bg-BG-second p-5 md:p-10 pt-9 md:pt-5 relative">
        ...
        <footer className="w-full flex flex-col-reverse md:flex-row items-center gap-3">
          <Button type="button" typeButton="regular-primary" label="Нет, оставить" onClick={no} />
          <Button type="button" typeButton="fill-primary" label="Да, удалить" onClick={ok} />
        </footer>
      </section>
    </div>
  )
