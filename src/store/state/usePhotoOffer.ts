...
export const usePhotoOffer = create<IUsePhotoOffer>((set, get) => ({
  ...
  dispatchPhotoOffer({ current, photos, payload, visible, author, offer }) {
    ...
    if (payload === "prev" && getCurrentIndex !== 0) {
      value.current = getPhotos?.find((item) => item?.index === getCurrentIndex - 1)
    }
    ...
    const getVisible = typeof visible === "undefined" ? get().visible : visible

    const getAuthor = typeof author === "undefined" ? get().author : value.author
